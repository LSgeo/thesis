...
Each method of gridding has characteristic interpolation traits (which when undesired are termed artifacts), but irrespective of the gridding process the detail resolvable in the output grid is a product of the sample height, sampling density, and the final cell size at which the grids are rendered.
...
Results are presented in \Cref{sec:2results} and are discussed in \Cref{sec:discussion}.
...
In addition to mean-square error, Feature Similarity (FSIM) \parencite{linzhangFSIMFeatureSimilarity2011} is used to quantify the quality of the super-resolved image, because it is a full reference image similarity metric that quantifies structural similarity using phase and local gradient magnitude components.
%TODO complain about FSIM
...
\Cref{fig:srdata1,fig:srdata2,fig:srdata3} contain open access grid data from the Eastern Goldfields Superterrane of Western Australia, prepared using the proposed method.
...
NW striking features around in HR are aliased in LR and subsequently incorrectly rotated by 90 degrees in the baseline SR prediction (\Cref{fig:srdata1}A).
The fine-tuned model (\Cref{fig:srdata1}B) resolves the correct strike, and separates the two features present.
