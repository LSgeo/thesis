...
Regions covered by high-resolution aeromagnetic surveys are biased toward prospective geological terranes. %TODO Cite
...
\Cref{fig:srdata} contains open access grid data from the Eastern Goldfields Superterrane of Western Australia, and prepared using the proposed method.
...
NW striking features around in HR are aliased in LR and subsequently incorrectly rotated by 90 degrees in the baseline SR prediction (\Cref{fig:srdata}A).
The fine-tuned model (\Cref{fig:srdata}B) resolves the correct strike, and separates the two features present.
