...
For example, ESRGAN uses an analogous upsampling network design to RDN (Residual Dense Network) shown in \Cref{fig:rdn}, with the addition of a discriminator network.
...
The generator (G) is analogous in design to the RDN architecture described above (\Cref{fig:rdn}), and is trained to upscale a low-resolution input, creating a super-resolution prediction.
...
\begin{figure}[hbtp]
    ...
    \caption[Power spectra for the test extent]{Radially Averaged Power Spectra for the complete test extent shown in \Cref{fig:trainvaldata}.
...
\begin{table}[hbtp]
    ...
    \begin{tblr}{
            ...
            row{1} = {guard}
                ...
                & \SetCell[c=2]{c} Bicubic &            & \SetCell[c=2]{c} RDN\textdaggerdbl{} &                 & \SetCell[c=2]{c} ESRGAN+              \\
        \hline{}
...
\begin{figure}[hbtp]
    ...
    \caption[Case study TMI grids and line profiles]{Jasper Hills Project TMI grids (a--e) and line profile (f).
...
The use of the state magnetic map at \qty{80}{\metre} and \qty{20}{\metre} for training data serves as a ready-made analogue for a low pass filter process, which the super-resolution networks successfully learned to reverse.
