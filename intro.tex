...
The Earth has a naturally occuring magnetic force field, approximating a dipole oriented toward the top and bottom of the planet.
Several properties of the magnetic field are important to the machine learning methods in this thesis.
First, that a field refers to a set of functions of space and time, and a vector field such as the magnetic field has three orthogonal components, with a function for each component.
Neural networks with sufficient parameters are recognised as universal function approximators \parencite{hornikMultilayerFeedforwardNetworks1989}, and should therefore be capable of approximating these functions of the magnetic field.
Second, the magnetic field has a force function with continuous derivatives, and is conservative \parencite{kelloggFoundationsPotentialTheory1967}.
The force function is known as the potential field, and the magnetic field is the gradient of the potential field.
This property is important later in this thesis, when calculating spatial derivatives and considering constraining neural networks with physics informed loss functions.

The magnitude of the magnetic field is the total magnetic intensity (TMI) \parencite{blakelyPotentialTheoryGravity1996}, with the unit nanotesla (nT).
Magnetic surveys in exploration geophysics generally create TMI products, and it may be measured directly or calculated from measurements of each orthogonal component.
...
Because of these known petrophysical variations in rock magnetisation, stratigraphies can be differentiated and delineated in magnetic maps, and their composition interpreted.
...
With the proposed method, scattered line data can be encoded into a neural network model, and grids created by querying cells at regular coordinates.
