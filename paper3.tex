...
Additionally, directional derivatives of the implicit representation calculated numerically and using automatic differentiation are shown, with average residuals of \SI{0.4} and \SI{0.8}{\nano\tesla\per\m} for the easting and northing derivatives respectively, albeit vertical derivative performance remains limited.
...
\Cref{fig:grid} shows the INR queried at \(z=\SI{40}{\m}\), which is the nominal terrain clearance of the survey.
...
Gradients calculated using the learnt representation are , and gridded after gradient calculation.
