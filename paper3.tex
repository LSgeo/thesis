...
When surveying these fields, which are continuous functions in three dimensions, sample locations are scattered in 3D space, despite best efforts to conform to a regularly spaced line plan.
...
Querying a learned INR using a regularly spaced \(x, y\) grid at a specific \(z\) height is a novel method for producing level grids from geophysical potential field surveys.
...
Sptaial gradients can be calculated directly from the learned representation, and can be queried at arbitrary locations.
