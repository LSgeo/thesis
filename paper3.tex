...
\begin{abstract}
    The recent use of spatial coordinate features in multilayer perceptron neural networks provides exigient opportunities for potential field geophysics.
    ...
    We further demonstrate the potential for additional processing of the learnt function in the continuous domain using automatic differentiation, with the same tool used to train the neural network.
    For example, directional gradients and subsequent filters are calculated prior grid quantisation, and therefore without the associated loss of high-resolution content during gridding.
...
\section{Method}
\subsection{Notation}
\(\mathbb{R}^n\) Real n-dimensional vectors, named using lower-case letters, e.g. \boldmath{u}.
