...
While horizontal derivatives queried from the INR correspond well to numerical derivatives calculated on the \qty{40}{\m} level grid in \Cref{fig:horigrad}, the vertical derivative suffers poor performance (\Cref{fig:vertgrad}).
...
\begin{figure}[hbt]
    ...
    \caption[Vertical derivative]{Vertical derivative grids queried from AD differentiated INR (left) and calculated numerically (right) on the \qty{40}{\m} level grid queried from the INR\@.
     The proposed method has poor vertical gradient representation, possibly due to the nature of aeromagnetic survey design.}
    \label{fig:vertgrad}
\end{figure}
