...
\title{Implicit neural representation of potential fields}
...
\begin{abstract}
    ...
    We present a novel method for implicit neural representation (INR) of potential fields, demonstrate the quality of the learned implicit function by encoding real airborne geophysical survey line data, predict constant level grids, and compare the result to grid data processed with a traditional workflow.
...
A classic example of an MLP is using a structured input vector of image pixel intensities \(u\), and an output vector of class labels \(l\).
...
Periodic activation functions differ from typical non-linear activations by scaling the response of the neuron with an activation function that repeats periodically, allowing for continuous differentiation throughout the depth of the network.
...
\begin{figure}[hbtp]
    \centering{}
    \includegraphics[width=\linewidth]{fig/p3/P864_sample_locs_raster.pdf}
...
Either a geographic or projected coordinate system can be used, provided for consistency between the query coordinates for inference and those used during training.
...
\begin{landscape}
    \begin{figure}[hbtp]
        ...
        \caption[Grid prediction at the nominal altitude]{Comparison between the reference grid processed by Geoscience Australia (left) and the constant level grid queried from the INR at \qty{40}{\m} (middle). The residuals are shown (right) for areas containing reference data, with a range of ±2 standard deviations of values in the reference grid.}
        \label{fig:grid40}
    \end{figure}
\end{landscape}
...
\begin{figure}[hbtp]
    ...
    \caption[Vertical derivative]{Vertical derivative grids calculated numerically on the \qty{40}{\m} level grid queried from the INR (left) and queried from AD differentiated INR (right).
        The proposed method has poor vertical gradient representation, possibly due to the low range of altitudes when sampling in aeromagnetic survey design.}
    \label{fig:vertgrad}
\end{figure}
...
Initial experiments with synthetic data evenly sampled across the vertical extent learn a vertical gradient with similar performance to the horizontal gradients, supporting this conjecture.
...
The current method could include additional target dimensions, which may allow representation learning of vector fields \(\mathbb{R}^{3}\) in three dimensions \(\mathbb{R}^{3}\).
...
\end{equation}
where \(S\) indicates the number of sample points in the field survey \(\phi(x, y, z)\), implicitly represented by the model \(f\).
