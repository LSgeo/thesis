...
\author{{Luke Thomas Smith} \and{Tom Horrocks} \and{Eun-Jung Holden} \and{Daniel Wedge} \and{Naveed Akhtar}}
...
Recent advances in CMLPs by using periodic activations such as sinusoids have demonstrated that high frequency information can be modelled by CMLP networks, while being continuously differentiable through the depth of the network to facilitate rapid training.
...
Due to using a continuous coordinate representation, there is no dependency on grid quantisation from an initial cell size selection.
It also provides for further processing that can be calculated analytically in the continuous spatial domain, allowing the use of all available sample information in these calculations.
...
The method is still in its infancy, but stands to benefit from ongoing research in the highly active fields of deep learning and INR\@.
...
\label{sec:inr}
CMLP networks are fully connected networks, which have been ubiquitous in machine learning for several decades.
...
CMLP networks for INR can be compared to compressed sensing \parencite{candesIntroductionCompressiveSampling2008} for its ability to recover accurate grids from sparsely sampled geophysics samples.
...
This method underpins modern machine learning frameworks such as Pytorch \parencite{paszkePyTorchImperativeStyle2019}, which is the framework used to construct the network architecture and perform training in this work.
In these frameworks, training is performed by evaluating a set of inputs using the current model weight parameters during a forward pass, while accumulating the partial derivatives of the result with respect to those parameters, to navigate an objective (loss) function using those gradients.
These gradients inform weight updates in the direction of the negative gradient, toward a minimum in the objective function.
...
\subsection{Data}
A case study uses data from the 2002 Wolfe Creek meteorite impact magnetic survey \parencite{wolfecreek2019}.
...
\qty{70}{\percent} of the sample data are used for training, and a random \qty{20}{\percent} of the remaining survey sample points are reserved to validate the model performance during training.
...
The Wolfe Creek meteorite impact crater case study line data has \SI{76420}{} sample points, and the Geoscience Australia provided reference grid has a resolution of 446 \(\times{}\) 445 cells.
...
While horizontal derivatives queried from the INR correspond well to numerical derivatives calculated on the \SI{40}{\m} level grid \cref{fig:hori_grad}, the vertical derivative suffers poor performance \cref{fig:vert_grad}.
...
\begin{figure}[hbt]
    ...
    \caption[Vertical derivative]{Vertical derivative grids queried from AD differentiated INR (left) and calculated numerically (right) on the \SI{40}{m} level grid queried from the INR\@.}
    \label{fig:vert_grad}
\end{figure}

\section{Discussion}
\label{sec:discussion}
INR has strong learning capacity for the potential field samples in the horizontal plane near the nominal survey altitude, allowing extraction of level grids which are accurate against the reference grid.
...
\[
    \nabla^2 \phi = 0
\]
In three dimensions, \cref{eqn:Laplace};
\begin{equation}
    \label{eqn:Laplace}
...
A geophysics informed network could include a Laplace criterion, \cref{eqn:cri_laplace}.

\begin{equation}
    \label{eqn:cri_laplace}
    \mathcal{L}_{PDE} = \beta{}\lVert{}\frac{\partial{}^2f}{\partial{}x^2} + \frac{\partial{}^2f}{\partial{}y^2} + \frac{\partial{}^2f}{\partial{}z^2}\rVert{}_{2}
\end{equation}

As denoted by the \(L^2\) operator \(\lVert{}\cdot{}\rVert{}_{2}\) and weighting \(\beta{}\), this criterion penalises outputs where the sum of partial derivatives is not close to zero.
